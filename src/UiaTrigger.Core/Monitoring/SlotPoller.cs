...
//
// <b>ライブラリの判断で回るものではない。</b>armed になるのは
// TriggerDefinition.PollInterval を明示的に指定したトリガーだけで、
// 指定が無ければ (既定) このクラスは<b>そもそも生成されない</b> = タイマーが 1 本も存在しない。
//
...
//                          回りっぱなしになる形が無い
//
// 周期タイマーにしていないのは、相手が TransactionTimeout ぶん止まったときに<b>周が重なる</b>
...
internal sealed class SlotPoller : IDisposable
{
    ...
    /// <param name="interval">
    /// 周と周の間隔。<b>正の値でなければならない</b> — 0 は「ポーリングしない」であって
