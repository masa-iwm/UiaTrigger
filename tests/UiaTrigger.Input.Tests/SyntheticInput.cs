...
// ヒットテストを**すべて通る**。物理入力と違うのは 3 点だけである:
...
/// <summary>
/// 最下層へ入る合成入力。**撃てなかったことと、届かなかったことを区別する。**
/// </summary>
/// <remarks>
/// <para>
/// どのメソッドも、OS が受け付けた本数が期待と違えば**その場で落ちる**。
/// これは docs/TESTING.md §4 の規律である — 「入力が届かなかった」と
/// 「**ハーネスが撃てなかった**」を混ぜると、対象の不具合とハーネスの不具合が
/// 同じ赤に見える。
/// </para>
/// <para>
...
internal static partial class SyntheticInput
{
    ...
    /// <remarks>
    /// <para>
    /// down と up を**1 回の呼び出しで**入れる。2 回に分けると、あいだに他の入力
    /// (人が触った実キーボード) が割り込みうる。
    /// </para>
    /// <para>
    /// **フォーカスは要らない。**低レベルフックはグローバルで、フォアグラウンドが
    ...
    /// </summary>
    /// <remarks>
    /// **座標はここでは指定しない。**狙いの検算 (期待矩形を 2 つの独立した情報源から
    ...
    /// </summary>
    /// <remarks>
    /// **必須の作法である** (docs/TESTING.md §3)。開発機で走らせるとマウスが取られるので、
    ...
    public sealed class CursorGuard : IDisposable
    {
        ...
        /// <summary>
        /// カーソルを置く。**置けたことを確かめてから返る。**
        /// </summary>
        /// <remarks>
        /// <c>SetCursorPos</c> は成功を返しても、別のフックやリモートデスクトップの
        /// カーソル制御で**実際には動かないことがある**。動いていないまま撃つと
    ...
    /// </summary>
    /// <remarks>
    /// **<c>MOUSEINPUT</c> は使わなくても書いておくこと。**共用体の大きさは最大のメンバーで
    /// 決まり、<c>INPUT</c> の大きさは <c>cbSize</c> として OS へ渡る。
    /// キーボードのぶんだけ書くと 64bit で 40 ではなく 32 になり、
    /// <c>SendInput</c> は**1 本も入れずに 0 を返す** — 「届かなかった」に見える形で
    /// ハーネスが壊れる。
    /// </remarks>
