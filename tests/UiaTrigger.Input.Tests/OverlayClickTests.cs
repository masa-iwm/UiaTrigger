...
//
// 枠線の上を合成クリックし、<b>下のアプリまで届く</b>ことを見る。実マウスでも
...
//
// 観測点は「ボタンが Click を上げたか」ではなく「<b>クリックが対象アプリまで届いたか</b>」。
// 前面化に食われた場合も<b>前面が変わる</b>ので、どちらでも「届いた」と数えられる。
// オーバーレイは WS_EX_NOACTIVATE なので、それ自身を押しても前面は変わらない —
// つまり前面が変わったなら、クリックはオーバーレイを通り越している。
// Click だけを観測点にすると<b>押す順で答えが変わる</b> (本題を 1 発目にすると
// 前面化に食われ、対照を先にするとオーバーレイが下へ回る) ので、この形にしてある。
// また、対照 (アルファ 0 の内側) が無いと「届かなかった」と「枠が塞いだ」を分けられない。
//
// <b>Skip にしないこと。</b>「走らないから緑」はこの repo が各所で塞いでいる形である。
...
public sealed class OverlayClickTests
{
    ...
    /// <summary>
    /// 枠線の上のクリックが<b>下のアプリまで届く</b>こと。届かなければ製品の欠陥である。
    /// </summary>
    /// <remarks>
    /// <para>
    /// <b>観測点は 2 つで、どちらか一方でも動けば「届いた」とする。</b>
    /// (a) ボタンの <c>Click</c> が上がる (b) <b>前面が対象アプリに変わる</b>。
    /// 前面でない窓を押した 1 発目は前面化に食われて (a) が上がらないことがあるが、
    /// <b>そのときも (b) は動く</b> — 前面化が起きた = クリックはその窓まで届いた、である。
    /// この 2 つを両方見ることで<b>押す順に依存しなくなる</b>。
    /// </para>
    /// <para>
    /// <b>撃つ前に狙いを検算する</b> (docs/TESTING.md §3 の解禁条件)。枠線の点で
    /// <c>WindowFromPoint</c> がオーバーレイを返すこと = そこでオーバーレイが対象アプリより
    /// <b>上に居る</b>ことを確かめる。<c>WindowFromPoint</c> が <c>HTTRANSPARENT</c> を
    /// 尊重しないのは実測済み (docs/DESIGN.md §10) で、不透明な枠の上ではこれは
    /// <b>Z オーダーだけを見ている</b> — ここではそれがちょうど欲しい情報である。
    /// </para>
    /// <para>
    /// 押す点は<b>枠の下辺</b>にする。利用者が実マウスで押したのがそこだからである。
    /// アイコンは右上なので、下辺は当たり判定から最も遠い。
    /// </para>
    /// <para>
    /// <b>対照は枠の内側 (アルファ 0) である。</b>あそこは Z オーダーに関係なく下へ抜けるので、
    /// 「そもそもこのハーネスはこのボタンを押せるのか」を切り分けられる。<b>本題を先に押す</b> —
    ...
    public void AClickOnTheFrame_ReachesTheApplicationBelow()
    {
        ...
        // <b>WindowFromPoint では確かめられない</b> (docs/DESIGN.md §10)。枠は WS_EX_TRANSPARENT で
        ...
        Assert.True(
            ...
            "下に回っていると、枠が塞いでいても素通りして<b>偽の合格</b>になります (docs/DESIGN.md §10)。" +
        ...
        // <b>対照を先に主張する。</b>ここが動いていなければ枠線の結果は解釈できない
        Assert.True(
            insideReach.Reached,
            "対照が成立しませんでした — 枠の内側 (アルファ 0) を押しても対象アプリに何も起きません。" +
            "これは「枠が塞いだ」ではなく<b>ハーネスがこのボタンを押せていない</b>ということです。" +
            Environment.NewLine + measured + Environment.NewLine + scenario.Describe());

        Assert.True(
            line.Reached,
            "<b>枠線の上のクリックが下のアプリまで届いていません。</b>" +
            "同じボタンを枠の内側から押すと届く (対照が成立している) ので、" +
            "「押せなかった」ではなく<b>枠が塞いでいる</b>ということです。" +
            "枠の窓は WS_EX_TRANSPARENT で<b>窓ごと</b>ヒットテストから外れている約束です " +
