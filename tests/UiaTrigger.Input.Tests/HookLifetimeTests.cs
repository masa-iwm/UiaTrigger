...
public sealed class HookLifetimeTests
{
    ...
    /// <summary>
    /// ピッカーを閉じた後は、←/→ で<b>枠が出ない</b>こと (MANUAL-CHECKS §2)。
    /// </summary>
    /// <remarks>
    /// <para>
    /// <b>先に「閉じる前は動く」ことを確かめる。</b>これを飛ばすと、
    /// 端に居て動きようがなかった場合と区別できない (<c>ArrowKeyTests</c> の
    /// ネガティブコントロールと同じ手順。実測で踏んだ形である)。
    /// </para>
    /// <para>
    /// <b>このテストの検出力について正直に書く。</b>計画は退行として
    /// 「<c>Dispose</c> から <c>UnhookWindowsHookEx</c> を落とす」を挙げていたが、
    /// <b>それでは落ちない</b> (実測 — docs/TESTING.md §3 の K3)。枠を出す経路は閉じるときに 3 つとも切れる:
    /// <c>ArrowKeyPressed</c> のハンドラーが外れ、登録表から消え
    /// (<c>Unregister()</c> は <c>_hook?.Dispose()</c> の<b>前</b>である)、
    /// ウィンドウ自体が壊される。フックの解除は<b>いちばん外側</b>にあり、
    /// 外から見える差を作らない。
    /// </para>
    /// <para>
    /// つまりここが固定しているのは「フックが解除されたこと」ではなく
    /// <b>「閉じたピッカーが二度と枠を出さないこと」</b>である。
    /// 実際に落とせる退行は <c>TriggerPickerPresenter.Dispose</c> の後始末を外す形で、
    /// それは docs/TESTING.md §3 (K3) に書いてある。<b>フックのハンドル自体が漏れているかは、
    /// 外からは観測できない</b> — そこは人の項目として残る (MANUAL-CHECKS §2)。
    /// </para>
    /// </remarks>
    [Fact]
    ...
    /// <summary>
    /// ←/→ が<b>フックを有効にしているピッカーだけ</b>に届くこと (MANUAL-CHECKS §6)。
    /// </summary>
    /// <remarks>
    /// <para>
    /// <b>2 段構えである。</b>先に両方 ON で撃って<b>2 つとも動く</b>ことを見る —
    /// これが<b>計数に検出力があること</b>の証明で、これを示さないと
    /// 後半の「1 つだけ動いた」が「たまたま 1 つしか動かなかった」と区別できない
    /// (<c>OverlayTests.OnePicker_ShowsExactlyOneOverlay</c> と同じ役割)。
    /// </para>
    /// <para>
    /// <b>どちらのピッカーを OFF にしたかは言わない。</b>ピッカーの窓とオーバーレイを
    /// 対応づける手段が無いので、「OFF にしたほうの枠が動かなかった」は書けない。
    /// 書けるのは<b>動いた枠の数</b>だけである。それで足りる —
    ...
    /// <summary>
    /// 枠が <paramref name="expected"/> 個だけ動くまで待ち、<b>それ以上動かないことも見る</b>。
    /// </summary>
    /// <remarks>
    /// <para>
    /// 「1 つ動いた」で止めると、<b>まだ 2 つ目が動く途中</b>だったときに通ってしまう。
    /// 数が揃ってから、短い窓のあいだ増えないことまで確かめる。
    /// </para>
    /// <para>
    /// <b>対応付けではなく数で見る。</b>どのオーバーレイがどのピッカーのものかを
    /// 言う手段が無いのは <c>OverlayTests</c> と同じ理由である。
    /// </para>
    /// <para>
    /// オーバーレイの<b>個数</b>が変わったときは待ち続けない — 枠が消えたのは
    /// 「動いた」ではなく<b>別の失敗</b>であり、数え方の前提が崩れている。
    /// </para>
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// 2 枚が同じ要素へ移ると矩形が一致しうるので、集合ではなく<b>個数</b>で数える。
    /// <b>塞げていない穴を書いておく</b>: 動いた先が、もう一方の<b>元の</b>矩形と
    ...
    /// <summary>
    /// ピッカーを 1 枚選んで自動選択を OFF にする。<b>OFF になったことを確かめてから返る。</b>
    /// </summary>
