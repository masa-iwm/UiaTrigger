...
public sealed class TriggerMonitorPollingTests
{
    ...
    /// 1 周ぶんの仕事は<b>キューに入っている</b>。同じキューへ後から投げたものを待てば、
    /// FIFO により前のものは済んでいる。
    /// </summary>
    ...
    /// </para>
    /// <para>
    /// <b>ここが言っているのは「解決済みのまま推移するかぎり」である。</b>
    ...
    /// <summary>
    /// <b>ポーリングを頼んだトリガーは「孤児」ではないこと。</b>
    ///
    /// <para>
    /// <see cref="SubscriptionRepair"/> の復旧の経路と、利用者が頼んだポーリングは<b>別のものである</b>。
    /// 前者はライブラリが自分の判断で回すので「壊れている間だけ」という厳しい条件が要り (docs/DESIGN.md §8)、
    /// 後者は利用者が頼んだ費用なので条件が要らない。似ているからといって片方をもう片方の
    /// 仕組みに載せると、<b>ポーリングを頼んだだけで復旧の再試行が armed になり、掃引が回りはじめる</b>。
    /// </para>
    /// <para>
    /// <b>この主張を名指しで固定しておく。</b>実際にやってみると
    /// <see cref="Polling_DoesNotDriveResolution"/> のほうが先に落ちる (掃引が増えるため) が、
    /// あれが落ちるのは<b>結果</b>であって、壊れたのがこの区別であることは名前からは分からない。
    ...
    /// <summary>
    /// <b>未解決のスロットは読まないこと</b> (周は回っても往復は 0 のまま)。
    ///
    ...
    /// <summary>
    /// <b>停止でポーリングが止まり、再開でまた張られること。</b>
    ///
    /// デバウンス予約を停止時に解いていないと再開後の掃引が黙って捨てられる、という
    /// A14 と同じ形の落とし穴である。こちらは向きが逆で、<b>止めたのに回り続ける</b>ほうが危ない。
    /// </summary>
    [Fact]
