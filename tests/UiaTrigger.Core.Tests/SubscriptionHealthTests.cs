...
///
/// <para>
/// この規則が<b>緩いほうへ</b>壊れると、まだ起動していないアプリのトリガーが常に
/// 「壊れている」と判定され、復旧の再試行が回りっぱなしになる。
/// <b>そうなっても機能は動いたままなので、他のテストは 1 件も落ちない</b> —
/// 「イベント駆動でポーリングしない」という設計の売りだけが静かに消える。
/// だから表で 4 つの場合すべてを固定する。
/// </para>
/// <para>
/// 逆に<b>厳しいほうへ</b>壊れると、購読を失ったトリガーが二度と拾われない
...
public sealed class SubscriptionHealthTests
{
    ...
    /// <summary>4 つの場合。<b>孤立と言えるのは 1 つだけである。</b></summary>
