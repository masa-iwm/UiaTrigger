...
//
// **これは実測から出た検査である。**デスクトップのアイコンの上で
// UIA のヒットテストが**その点を含まない要素**を返すことを実測した
// (点 (150,47) に対して (0,5)-(100,70) の「ごみ箱」)。ピッカーはそれを忠実に表示するので、
// カーソルを隣のアイコンへ動かしても選択が変わらない。
//
// **UiaSession の側は T1 から回せない** (実 UIA が要る)。ここで固定できるのは
...
{
    /// <summary>
    /// 矩形は**左上を含み、右下を含まない** (Win32 の半開区間)。
    /// </summary>
    /// <remarks>
    /// **辺の扱いがここで効く。**デスクトップのアイコンは実測で
    /// (0,5)-(100,70) と (100,5)-(200,90) のように**辺を共有して並ぶ**。
