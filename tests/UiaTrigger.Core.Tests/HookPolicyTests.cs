...
/// <summary>
/// 低レベルキーボードフックが<b>入力の出どころで分岐しない</b>ことの不変条件
/// (docs/DESIGN.md §11 / docs/TESTING.md §3)。
///
/// <para>
/// 合成入力を使うテスト層 (T5) がある (docs/TESTING.md §3)。実測では、
/// 合成した入力には<b>例外なく</b>注入ビットが立つ (キー 2/2・マウス 1/1)。つまり
/// <c>HookProc</c> がそのビットで分岐した瞬間、<b>T5 と物理入力は必ず別の経路を通る</b>。
/// </para>
/// <para>
/// どちらへ壊れるかは分岐の向きで決まる。注入を<b>捨てる</b>分岐なら T5 が赤くなる
/// (見える壊れ方)。注入だけを<b>通す</b>分岐なら <b>T5 は緑のまま実機が死ぬ</b> —
/// docs/TESTING.md §4 のホイールバグ (ハーネスが被験体の欠陥を共有していた) と<b>まったく同じ形</b>が
/// 1 層上で再現する。この不変条件は向きに依らず両方を塞ぐ。
/// </para>
/// <para>
/// <b>綴りではなく参照そのものを見る。</b><c>NativeMethods.txt</c> は
/// <c>KBDLLHOOKSTRUCT</c> しか挙げておらず、注入ビットの定数は生成されていない可能性がある。
/// 定数名で探す正規表現は<b>空振りして緑になる</b> — 塞ぎたかったのと同じ失敗形である。
/// </para>
/// <para>
/// <b>塞げていない穴を書いておく</b>: 見るのは <c>OverlayController.cs</c> だけである。
...
public sealed class HookPolicyTests
{
    ...
    ///
    /// <para>
    /// <b>「無いこと」の主張なので、探し損ねても緑になる。</b>そこで先にメソッドを
    ...
    /// <summary>
    /// 同じファイルの<b>どこか</b>で構造体のフラグが読まれていないこと。
    ///
    /// <para>
    ...
    private static string ReadHookProc(string source)
    {
        ...
        //
        // <b>目印は「HookProc より後ろに在り、かつ flags を綴るメンバー」へ向けること。</b>
        // 目印にしたメンバーがリファクタリングで消えると<b>この検査は何も守らなくなる</b>
