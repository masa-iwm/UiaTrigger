...
/// **実時間を一切待たない**。間隔を待って確かめると「待ち時間を延ばしただけのテスト」になり、
/// 値そのものを固定できない。
/// </para>
/// <para>
/// この 2 つは形が似ているが別のものである。<see cref="SubscriptionRepair"/> は
/// **ライブラリが自分の判断で**回す復旧の経路なので「壊れている間だけ」という厳しい条件が要り、
/// backoff も要る。<see cref="SlotPoller"/> は**利用者が頼んだ費用**なので、
...
public sealed class SlotPollerTests
{
    ...
    /// <summary>
    /// **間隔は毎周同じであること** (<see cref="SubscriptionRepair"/> と違って伸びない)。
    ///
