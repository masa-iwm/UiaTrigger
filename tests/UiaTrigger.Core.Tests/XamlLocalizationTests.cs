...
public sealed class XamlLocalizationTests
{
    ...
    /// 表示文字列の直書きを <c>.cs</c> 側で見るか。<b>ラベルをコードで代入する変種はすべて対象</b>で、
    ...
    ///
    /// 番号の取り違えは<b>実行するまで分からず、しかも例外にならない</b>
    ...
    /// XAML があるかどうかではなく<b>ラベルをどこで入れるか</b>で対象を決める。
    /// <c>Text = "…"</c> と書けばそのまま画面に出るし、ビルドも通る。
    /// 表示に関わるプロパティへの<b>文字列リテラルの代入</b>を探して塞ぐ。
    /// </para>
    /// <para>
    ...
    ///
    /// x:Uid の綴り違いは<b>例外にならず、その要素の文字列が空のまま表示される</b>。
    ...
    /// <b>ビルドは通り、例外も出ない</b> — 画面の文字列がキー名に化けるだけである。
    ///
    ...
    /// <b>全件</b>ではなく「大半が違う」ことを見る。コピーしただけのファイルを弾ければ足りる。
    /// </summary>
    [Theory]
    [MemberData(nameof(WithResources))]
