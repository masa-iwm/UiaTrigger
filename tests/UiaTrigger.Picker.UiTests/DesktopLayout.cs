...
//
// 対象アプリの窓とホストの窓が<b>重ならない</b>ことを、ここ 1 箇所で決める。
...
/// <remarks>
/// <para>
/// <b>ここが T4 の非決定性の出どころである。</b>実測 (1600x900):
/// 対象アプリ (1228,471)-(1552,692) に対して pick 点は (1350,560)、
/// WPF のピッカーは (52,52)-(1152,752) で<b>届かない</b>が、
/// WinUI のピッカーは (156,156)-(1356,774) で<b>pick 点を 6px 覆っていた</b>。
/// つまり「WPF は安定していて WinUI は不安定」ではなく、
/// <b>WPF の窓が 1100 幅で偶然届かなかっただけ</b>である。
/// </para>
/// <para>
/// 覆われるとピッカーは自プロセスを掴んで捕捉を飛ばし、しかも
/// <c>TriggerPickerPresenter.TickAsync</c> は <c>_lastCapturedPoint</c> で
/// 「同じ点は再捕捉しない」ので<b>二度と捕捉しない</b>。
/// 症状は 20 秒のタイムアウトで、原因は「実体化しなかった」に見える。
/// </para>
/// <para>
/// 1600x900 の画面に 1200x618 の窓が任意のカスケード位置で出る以上、
/// <b>静的に安全な領域は存在しない</b>。だから当てにするのをやめ、
/// ホストの窓を<b>出た瞬間に退かす</b> (<see cref="PickerHostProcess"/>)。
/// </para>
/// </remarks>
internal static class DesktopLayout
{
    ...
    /// <summary>対象アプリの窓。<b>右下</b>に置く。</summary>
    public static (int Left, int Top, int Width, int Height) Target { get; }

    /// <summary>ホストの窓 (メインもピッカーも)。<b>左に寄せる</b>。</summary>
    public static (int Left, int Top, int Width, int Height) Host { get; }

    /// <summary>
    /// <b>幅を固定した</b>ホストの窓。表示域の広さそのものを検査対象にするテスト用。
    /// </summary>
    /// <remarks>
    /// <para>
    /// <see cref="Host"/> の幅は <c>Target.Left - Gap</c> で、<b>デスクトップの広さで変わる</b>。
    /// 「深いチェーンで水平スクロールが要る」のような<b>内容が表示域に収まるかどうか</b>を
    /// 見るテストは、これに乗ると機械ごとに結果が変わる — 実際 3840x2160 では
    /// ホスト幅が 3436px になり、30 段の字下げが収まってしまって
    /// <b>検出力を失ったまま赤い</b>状態になっていた (docs/DESIGN.md §9)。
    /// </para>
    /// <para>
    /// <b>幅には上と下の両方から制約がある。</b>狭すぎても成立しない —
    /// このテストの対照は「畳めば水平スクロールが要らなくなる」なので、
    /// <b>畳んだときに残る 1 行が収まる</b>幅は要る。
    /// </para>
    /// <para>
    /// 実測 (175%、ツリーはホスト幅の約 52%):
    /// </para>
    /// <list type="table">
    /// <item><description>ホスト 3436px → ツリー 1838px: 30 段でも<b>表示率 100%</b> (収まってしまう = 検査にならない)</description></item>
    /// <item><description>ホスト 700px → ツリー 346px: 30 段で表示率 70.9% だが、<b>畳んでも水平スクロールが残る</b> (対照が壊れる)</description></item>
    /// </list>
    /// <para>
    ...
    /// <b>1100px (ツリー約 570px) は、どちらの表示スケールでも両方の条件を満たす。</b>
    /// </para>
    /// </remarks>
    ...
    /// <summary>
    /// 割り付けそのものが成立していること。<b>画面が狭いと成立しない。</b>
    /// </summary>
    /// <remarks>
    /// ハーネス自身の検証である (docs/TESTING.md §4 の教訓 (b))。ここが成立していなければ
    /// 「ホストの窓を退かした」あとも pick 点が覆われうるので、
    /// 落ちるべきは個々のテストではなく<b>割り付け</b>である。
    /// </remarks>
    /// <summary>
    /// <b>2 つ目の</b>対象アプリの窓。<see cref="Target"/> の<b>真上</b>に積む。
    /// </summary>
    /// <remarks>
    /// <para>
    /// 複合条件 (docs/DESIGN.md §4) は<b>別プロセスを 2 つ</b>立てないと何も確かめられない —
    /// 同一ウィンドウ内の 2 要素だと、スロットが割れていても素通りするからである。
    /// </para>
    /// <para>
    /// <b>横ではなく縦に積む。</b>横に並べるとホストの幅 (<c>Target.Left - Gap</c>) を削ることになり、
    /// <see cref="NarrowHost"/> を含む既存の割り付けが全部動く。
    /// 縦なら <see cref="Host"/> も <see cref="Target"/> も 1 ピクセルも変わらない。
    /// </para>
    /// <para>
    /// <b>重ならないことが要点である。</b>記録は座標 (<c>ElementFromPoint</c>) を通るので、
    ...
    /// <summary>
    /// 2 つ目の対象アプリを置く余地があること。<b>画面が狭いと成立しない。</b>
    /// </summary>
    /// <remarks>
    /// <see cref="RequireItFitsOnThisScreen"/> と同じ理由でここに置く —
    /// 収まらないときに落ちるべきは個々のテストではなく<b>割り付け</b>であり、
    ...
    /// <summary>
    /// <see cref="NarrowHost"/> の割り付けが成立していること。<b>画面が狭いと成立しない。</b>
    /// </summary>
    /// <remarks>
    /// <para>
    /// <see cref="RequireItFitsOnThisScreen"/> は <see cref="Host"/> しか見ないので、
    /// これとは別に要る。<see cref="NarrowHost"/> は幅が固定 (1100px) なので、
    /// 画面が狭いと対象アプリの窓の上まで伸びて <b>pick 点を覆う</b>。
    /// 覆われるとピッカーは自プロセスを掴んで捕捉を飛ばし、二度と捕捉しない。
    /// </para>
    /// <para>
    /// <b>CI のランナー (1024x768 — docs/RELEASING.md §2) では実際に起きる。</b>
    /// 素通しすると症状は「ホストのメインウィンドウを pick 点から退かす が
    /// 30 秒以内に成立しませんでした」という<b>原因を名指ししないタイムアウト</b>になり、
    /// 画面の狭さは窓の矩形から逆算しないと見えない。
    /// </para>
    /// <para>
    /// <b>この画面では成立しない、と言えるだけでよい。</b>直す方法 (解像度を上げる) は
