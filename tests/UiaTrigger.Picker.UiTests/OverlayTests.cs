// S1: オーバーレイを<b>外から</b>観測する層 (docs/DESIGN.md §10)。
//
// ここでしか見られないものが 2 つある:
//   - 2 枚のピッカーが<b>それぞれ独立に</b>枠を出すこと。A18 で static singleton を
//     やめた効果は、T1 の fake View では見えない (オーバーレイは実ウィンドウである)
//   - 確定アイコンのピクセルの上にオーバーレイが居ること。他のトップモーストウィンドウが
//     帯の順位を主張すると、枠は見えたまま<b>アイコンだけが押せなくなる</b>
//
// WPF だけで回す。OverlayController は Picker.Core に在って 3 変種で<b>同じもの</b>なので、
...
public sealed class OverlayTests
{
    ...
    /// <summary>
    /// 枠が 1 枚のとき、オーバーレイは<b>枠 1 つとアイコン 1 つ</b>。
    /// </summary>
    /// <remarks>
    /// <para>
    /// 2 枚の対照実験である。<b>数えることに検出力があること</b>をここで示しておかないと、
    /// 「2 つある」という主張が「たまたま 2 つ見えた」と区別できない。
    /// </para>
    /// <para>
    /// <b>1 枚のピッカーは窓を 2 つ出す</b> (docs/DESIGN.md §10)。枠とアイコンを別の窓に
    ...
    /// <summary>
    /// 2 枚のピッカーが、<b>それぞれ自分の pick 点の要素</b>に枠を出すこと。
    /// </summary>
    /// <remarks>
    /// <para>
    /// <b>assert は集合の一致で書く。</b>オーバーレイにはタイトルも AutomationId も無く
    /// クラス名は共通なので、どの枠がどのピッカーのものかを言うテストは書けない。
    /// 期待側は <c>OverlayGeometry</c> (純関数) から計算する — マジックナンバーを書くと
    /// 幾何が変わったときに<b>テストだけが古い正解を守り続ける</b>。
    /// </para>
    /// <para>
    /// <b>1 回のサンプルでは見ない。</b>枠は 1 回の選択で 2 度描かれる
    ...
    /// <summary>
    /// 1 枚目を閉じても、2 枚目の枠は<b>元の場所に生き残る</b>こと。
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// <b>「どちらが残ったか」は言わない。</b>ピッカーの窓とオーバーレイを対応づける手段が
    /// 無いのと同じ理由で、閉じたのがどちらの窓なのかもテストからは決められない
    /// (窓を探す条件は「要素ツリーを持っていること」で、2 枚とも当てはまる)。
    /// 言えるのは「1 つ残り、それは<b>2 つの期待位置のどちらか</b>である」まで。
    /// </para>
    /// <para>
    /// これでも狙いは果たす: 共有ウィンドウへ戻せば<b>0 個</b>になって落ちるし、
    ...
    /// <summary>
    /// 別のトップモーストウィンドウが帯の順位を主張しても、<b>描き直せば</b>
    ...
    /// 枠は見えたままで、<b>確定アイコンだけが押せなくなる</b> — 例外は出ない。
    /// </para>
    /// <para>
    /// <b>「描き直せば」が付くのは正直に書いた制限である。</b>直しは
    /// <c>Redraw()</c> で最前面を主張し直すもので、再描画が起きるまでは下に居たままになる。
    /// 実際の操作ではホバーのたびに描き直すので実用上は戻るが、
    /// <b>瞬時に戻るわけではない</b>。ここではツリーの別の行を選んで描き直させている。
    /// </para>
    /// <para>
    /// 見る点は<b>アイコンの内側</b>である。レイヤードウィンドウのヒットテストは
    ...
    public void TheConfirmIconStaysOnTop_AfterAnotherTopmostWindowClaimsTheBand()
    {
        ...
        // 前後関係を見る前に、<b>そもそも競争が起きていること</b>を確かめる。
    ...
    /// <summary>
    /// 枠の 3 点の上に<b>誰が居るか</b>。アイコンと内側だけを固定する
    /// (docs/DESIGN.md §10)。
    /// </summary>
    /// <remarks>
    /// <para>
    /// 実機 (100%) で「<b>枠の上のクリックが下のアプリへ抜けていない</b>」という報告がある。
    /// 「まず実マウスで同じ点を押して比べる」(docs/TESTING.md §3) の最初のデータ点がこの測定である。
    /// </para>
    /// <para>
    /// <b>3 点を測った (100% / DPI 96)</b>。枠線 → <b>オーバーレイ</b> /
    /// アイコン → <b>オーバーレイ</b> / 枠の内側 → <b>対象アプリのボタン</b>。
    /// </para>
    /// <para>
    /// <b>枠線の結果からは何も結論できない。</b>較正して確かめた:
    /// <c>WM_NCHITTEST</c> が<b>全点で</b> <c>HTTRANSPARENT</c> を返すようにしても、
    /// アイコンの点はオーバーレイを返し続けた。つまり
    /// <b><c>WindowFromPoint</c> は <c>HTTRANSPARENT</c> を尊重しない</b> —
    /// <c>NativeWindows.WindowAt</c> の doc が書いていたとおりで、これは測定器の性質である。
    /// 枠線に assert を書くと<b>製品ではなく測定器を固定する</b>ことになるので書かない。
    /// </para>
    /// <para>
    /// 固定するのは 2 つだけである: アイコンはオーバーレイのもの (押せること)、
    /// 枠の内側は下のアプリのもの (アルファのヒットテストが効いていること)。
    /// 枠線の値は<b>失敗時の診断として</b>残す。
    /// </para>
    /// <para>
    /// <b>クリックスルーは依然として手動確認のままである</b> (docs/MANUAL-CHECKS.md §3)。
    /// 冒頭の報告を裏付けも否定もできていない。
    /// </para>
    /// <para>
    /// <b>ホバー追随が固定されている点に注意。</b><c>--pick-at</c> は <c>ICursorSource</c> を
    /// 固定するので、<c>IsInIconRect</c> が読む <c>_pendingRect</c> は動かない。
    /// <b>カーソルが動いている間だけ起きる取り違えは、この測定では原理的に出ない。</b>
    /// </para>
    /// </remarks>
    [Fact]
    public void TheOverlayOwnsTheIconPixel_ButNotTheTransparentInside()
    {
        ...
        // 枠の<b>左辺</b>の中央。不透明な赤線の上で、アイコン (右上) からは最も遠い。
        ...
        Assert.True(
            atIcon == iconHwnd,
            "確定アイコンの点がアイコンの窓のものではありません = <b>押せない</b>側の欠陥です。" +
        ...
        // 枠線については<b>何も主張しない</b>。較正で、WindowFromPoint が
        // HTTRANSPARENT を尊重しないことを実測しているためである (docs/DESIGN.md §10):
        // 全点で HTTRANSPARENT を返すようにしても、アイコンの点はオーバーレイを返し続けた。
        // ここで atLine に assert を書くと、<b>製品ではなく測定器の性質を固定する</b>ことになる。
        //
        // <b>2 窓構成 (docs/DESIGN.md §10) でも主張しない。</b>枠は WS_EX_TRANSPARENT で
        // 「窓ごとヒットテストから外れている」ので WindowFromPoint が枠を返さない
        // 可能性はある。だが<b>それは測定器の話であって、クリックが下へ届くかどうかとは別である</b> —
    ...
    /// <summary>
    /// <b>枠だけがヒットテストから外れていて、アイコンは外れていないこと</b> (docs/DESIGN.md §10)。
    /// </summary>
    /// <remarks>
    /// <para>
    /// クリックスルーの直しの本体は <c>WS_EX_TRANSPARENT</c> という<b>拡張スタイル 1 つ</b>で、
    /// 付け外しは 1 語の変更で済む。<c>OverlayClickTests</c> (T5) が実際に押して確かめているが、
    /// あれが見ているのは<b>枠の側だけ</b>である。
    /// </para>
    /// <para>
    /// <b>逆向きの壊れ方 — アイコンにも付けてしまい、確定アイコンが押せなくなる — を
    /// 機械で捕まえるのはここだけである。</b>合成入力で確定を押す検査は無く、
    /// 目視 (MANUAL-CHECKS §3 の「見えているアイコンは全面が押せる」) が一次の網である。
    /// ここで<b>原因のほう</b>を固定しておく。<c>TheOverlayWindowClass_RegistersACursor</c> が
    /// カーソルの見た目ではなくクラスの登録を見ているのと同じ規律である。
    /// </para>
    /// <para>
    /// 退行: 枠から <c>WS_EX_TRANSPARENT</c> を外す → 前半が落ちる (T5 も落ちる)。
    /// アイコンに付ける → 後半が落ちる (<b>T5 は緑のままである</b>)。
    /// </para>
    /// </remarks>
    [Fact]
    public void OnlyTheFrame_IsTransparentToHitTesting()
    {
        ...
        Assert.True(
            ...
            "この状態では<b>確定アイコンが押せません</b> — 窓ごとヒットテストから外れるためです。");
    }

    /// <summary>
    /// オーバーレイのウィンドウクラスが<b>カーソルを持っている</b>こと。
    /// </summary>
    /// <remarks>
    /// <para>
    /// MANUAL-CHECKS §4.3.1 の「確定アイコンにカーソルを重ねても砂時計にならない」を引き取る。
    /// <c>WNDCLASSEXW.hCursor</c> が <c>default</c> のままだと、そのクラスのウィンドウの上で
    /// カーソルが<b>直前の状態を引きずる</b> — ピッカーが読み取り中なら砂時計のままになる
    /// (実際に起きた不具合である)。
    /// </para>
    /// <para>
    /// <b>これは「カーソルの見た目」ではなく「クラスがカーソルを登録しているか」の検査である。</b>
    /// 実際にどう描かれるかは実マウスを動かさないと見られないので、
    /// MANUAL-CHECKS の項目は<b>補助として残す</b>。ここで捕まえるのは、
    /// 登録が落ちるという<b>原因のほう</b>である。
    /// </para>
    /// <para>
    /// <b>見るのはアイコンの窓のクラスである</b> (docs/DESIGN.md §10)。カーソルが実際に「乗る」のは
