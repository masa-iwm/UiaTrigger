// T4: 条件欄が<b>横にスクロールできる</b>こと (docs/DESIGN.md §12)。
//
// <b>ここでしか見られない。</b>WinUI3 の View は T1 から組み立てられず (docs/DESIGN.md §12)、WPF に写して
// 見ることもできない — あちらの条件欄の行は WrapPanel なので<b>折り返す</b>ので、
...
public sealed class ConditionPaneTests
{
    ...
    /// <remarks>
    /// <para>
    /// 条件欄の行は <c>Orientation="Horizontal"</c> の <c>StackPanel</c> で<b>折り返さない</b>。
    /// いちばん広い行は 724px あり、既定の窓 (1100px 幅 / 右側は 5/11) では必ずはみ出す。
    /// <c>HorizontalScrollMode</c> が <c>Disabled</c> だと<b>右が黙って切れる</b> —
    /// 例外も、切れたという表示も出ない (実測)。
    /// </para>
    /// <para>
    /// <b>負の対照は置けない。</b>「広ければ横バーは出ない」を見るには条件欄を 724px より
    /// 広げる必要があり、それには区切りを掴むか窓を広げるかで<b>合成入力が要る</b> (T5 の担当)。
    /// つまりこのテストが言えるのは「狭いときに横へ動ける」ことだけである。
    /// <b>「広い窓でも常に横バーが出る」= 折り返しが死んだ形は、ここでは捕まらない</b> —
