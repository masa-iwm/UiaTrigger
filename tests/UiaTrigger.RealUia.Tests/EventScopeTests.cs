...
///
/// <para>
/// このファイルは <b>全件を両プロファイル (WinForms / WPF) で走らせる</b>。
...
public sealed class EventScopeTests
{
    ...
    ///
    /// <b>同じ形の構造変化 (子を 1 個足す) を経路の内と外で起こして比べる</b>。
    ...
    /// <summary>
    /// ネイティブ UIA プロバイダーでは、対象の<b>削除</b>が経路購読に届くこと。
    ///
    /// <para>
    ...
    ///
    /// <para>
    /// <b>ここが動かないなら B3 の実装が誤っている。</b>
    /// 「親 1 段だけを購読する」案を却下した理由そのものでもある (docs/DESIGN.md §6)。
    /// </para>
    /// <para>
    /// ネイティブプロバイダーでは、切り離された部分木の中では親子関係が保たれたまま
    /// 応答し続けるため、<b>属性の突合でも上向きの navigation でも検出できない</b> (docs/DESIGN.md §8)。
    ...
    /// CPU 使用率は測れないが、<b>受信件数 0</b> はその必要条件であり、
