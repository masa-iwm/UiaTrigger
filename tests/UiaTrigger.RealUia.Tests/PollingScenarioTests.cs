...
/// 要素を 1 つも解決しない。<b>「読み直して、変わっていたら鳴る / 変わっていなければ鳴らない」は
/// 実際の要素が要る</b>ので、こちらの担当である。
/// </para>
/// <para>
/// <b>この一群の主眼は「鳴りすぎないこと」である。</b>ポーリングを足したときに最も起きやすい
/// 壊れ方は「動かない」ではなく<b>毎周期鳴る</b>で、しかもそれは
...
public sealed class PollingScenarioTests
{
    ...
    /// <summary>
    /// <b>まず測る: <c>set-accessible-name</c> は UIA の変化通知を上げないこと。</b>
    ///
    /// <para>
    /// これはポーリングのテストではなく、<b>あとの 2 件が何かを証明できるようにするための実測</b>である。
    /// この前提が崩れていると (= 実は通知が上がっていると)、「ポーリングが気づいた」と思っている
    /// 発火が、ただのイベント駆動の発火になる。<b>その状態でもテストは緑になる</b> —
    /// 「assert を書く前に測る」(docs/TESTING.md §3) と同じ理由でここに置く。
    /// </para>
    /// <para>
    /// 対になる肯定側も同じテストの中で見る。<c>set-text</c> は<b>通知を上げる</b>ので、
    ...
    /// <summary>
    /// <b>同じことを WPF (ネイティブ UIA プロバイダー) でも測る。</b>
    ///
    /// <para>
    /// 上の実測は WinForms (MSAA ブリッジ) のものである。<b>そのまま WPF に当てはめてはいけない</b> —
    /// 2 つのプロバイダー経路は非対称である (A21 — docs/DESIGN.md §6)。ネイティブプロバイダーは
    /// peer のプロパティを自動で突き合わせて通知を上げる仕組みを持つので、
    /// <b>WPF では「無通知の変化」を作れない可能性がある</b>。
    /// </para>
    /// <para>
    /// <b>どちらに転んでもこのテストは意味を持つ。</b>通知が上がらないなら WinForms と同じで、
    /// ポーリングの価値がプロバイダーによらないことが言える。上がるなら
    /// 「無通知の変化を構成できるのは MSAA ブリッジ側だけ」という知見であり、
    /// それ自体がプロバイダー差の記録 (docs/DESIGN.md §6) に連なる。<b>推測で書かず、実測を assert にする。</b>
    /// </para>
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// <b>ポーリングが、通知の上がらない変化に気づくこと</b> (この機能の存在理由)。
    ///
    ...
    /// <b>ポーリングがそれに気づくことも両プロバイダーで確かめる。</b>
    ///
    /// ポーリング自体はスナップショットを読み直すだけでプロバイダーに依存しないが、
    /// A21 (docs/DESIGN.md §8) が禁じているのは<b>まさにその「プロバイダーに依存しない」という推論</b>である。
    /// 1 本足しておく。
    /// </summary>
    [Fact]
    ...
    /// </para>
    /// <para>
    /// 観測窓のあとに<b>ポーリングが実際に回っていたこと</b>を診断で確かめる。
    ...
    /// <summary>
    /// <b>通知が上がる変化では、ポーリングを足しても発火が二重にならないこと。</b>
    ///
    ...
    /// <summary>
    /// <b>Custom 句を持つトリガーが、ポーリングで毎周期鳴らないこと。</b>
    ///
    /// <para>
    /// <see cref="TriggerProperty.Custom"/> はスナップショットに入らないので、
    /// <c>WatchedValuesChanged</c> は前回値を持たず<b>無条件に「変化あり」と答えていた</b>。
    /// イベント経路ではそれでよい (UIA が何かの変化を伝えてきている) が、
    /// ポーリング経路には手掛かりが無いため、そのままだと<b>これだけが毎周期鳴る</b> —
    ...
    /// <b>通るのは Custom の経路</b>であり、しかも実 UIA で安定している。
    /// </para>
    /// </summary>
    [Fact]
    ...
    /// <b>解決済みの要素を抱えていても回らない</b>ことは、ここでしか言えない。
    /// </summary>
    [Fact]
