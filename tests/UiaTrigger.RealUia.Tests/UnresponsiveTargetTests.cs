...
// UI スレッドを塞ぐ hang コマンドで<b>決定的に</b>作れる。
//
// <b>ただし、手動チェックの項目がそのままの形で全部成立するわけではない</b> (実測)。
...
public sealed class UnresponsiveTargetTests
{
    ...
    /// <summary>
    /// 塞がれていない側の発火を待つ期限。<b>この数字がこのテストの検出力そのものである。</b>
    /// </summary>
    /// <remarks>
    /// <para>
    /// <see cref="Hang"/> より<b>ずっと短く</b>すること。monitor が全体として直列化していれば
    /// 発火は塞ぎが明けるまで (10 秒) 遅れるので、短い期限だけがその失敗を捕まえる。
    /// </para>
    /// <para>
    /// <b><see cref="MonitorHarness.DefaultTimeout"/> (15 秒) で待ってはいけない。</b>
    /// 完全に直列化した実装でも 10 秒で発火して通ってしまい、<b>何も主張しないテスト</b>になる。
    ...
    /// (docs/MANUAL-CHECKS.md §8)。<b>1 プロセスでは言えない</b> — アプリが固まれば
    /// そのアプリのトリガーは全部固まるので、対象アプリを 2 つ起動して比べるしかない。
    /// </para>
    /// <para>
    /// <b>「<c>TransactionTimeout</c> を hang より長くすれば B も止まる」という形の
    /// ネガティブコントロールは成立しないので入れていない</b> — 実測では
    /// 1 秒でも 30 秒でも B は同じ速さで発火する。<b>B の発火経路は A を一度も通らない</b>
    /// ためで、呼び出し上限時間は「A を読みに行った呼び出し」を救う設定でしかない。
    /// 代わりに<b>期限</b>が検出力を持つ (<see cref="FireDeadline"/> の remarks)。
    /// </para>
    /// <para>
    ...
    public async Task WhileOneAppIsUnresponsive_TheOtherAppsTriggerKeepsFiring()
    {
        ...
        // 塞ぎの<b>後</b>に確かめても、観測した窓のあいだ塞がっていた証拠にならない
        appA.Post("ping");
    ...
    /// <summary>
    /// 応答しない状態が、そのアプリのトリガーの解決を<b>恒久的に汚染しない</b>こと。
    /// </summary>
    /// <remarks>
    /// <para>
    /// MANUAL-CHECKS §8 の「停止したアプリのトリガーは未解決になり、
    /// 復帰後に再解決される」を<b>到達できる形へ組み替えたもの</b>である。
    /// </para>
    /// <para>
    /// <b>前半は実測で成立しない。</b>解決済みのトリガーは 10 秒塞いでも未解決にならない
    /// (<c>resolved=2/2</c> のまま — docs/DESIGN.md §3)。これは「揺れない」という意味で
    /// むしろ正しい振る舞いであり、無理に未解決を作って assert すると<b>実装が持っていない
    /// 性質を要求する</b>ことになる。
    /// </para>
    /// <para>
    /// 代わりに<b>先に要素を消して未解決にしてから塞ぐ</b>。塞いでいるあいだ monitor は
    /// A のツリーを歩き直そうとして失敗し続ける。明けてから要素を戻すと解決し、発火する。
    /// 「塞がれたことが解決の経路を壊したままにしない」が言える形になっている。
    /// </para>
    /// <para>
    /// 退行: 塞がれた相手への呼び出しが上限時間で諦めない実装 (あるいは諦めた後に
    /// 解決を再試行しない実装) にすると、要素を戻しても解決せず期限で落ちる<b>はずである</b>。
    /// <b>これは未検証である</b> — 簡単に注入できる形の退行が無い。
    ...
    /// UIA の既定 (20 秒) まで返らない。<b>その状態で緑になっても何も言えない</b>。
    /// </remarks>
    ...
    /// <summary>
    /// 2 つの対象アプリを<b>重ならない場所</b>へ置き、それぞれに 1 個ずつボタンを足す。
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// <b>ここで窓を縮めてよいのは、このシナリオがボタンを 1 個しか置かないためである。</b>
    ...
    /// </para>
    /// <para>
    /// <b>ボタンの名前を 2 つで変えてある</b> (<c>btnA</c> / <c>btnB</c>)。同じ実行ファイル・
    /// 同じクラス名なので、名前まで同じにすると<b>取り違えた定義が静かに解決してしまい</b>、
    ...
    /// </summary>
    /// <remarks>
    /// <b>ハーネス自身の検証である</b> (docs/TESTING.md §1 の教訓 (b))。塞ぐ<b>前</b>に送っておいた
    /// <c>ping</c> をここで受ける。塞ぎが効いていなければ即座に返っているので、
    /// 経過時間が短ければ「もう片方が発火した」は<b>何も言っていない</b> —
    /// 応答するアプリの隣で発火しただけである。
    /// </remarks>
