...
分類 2 の例外メッセージには**もう 1 つ区別がある。利用者に出る例外はリソース経由、
プログラマ向けの契約違反は英語直書き**である。後者は「呼び出し方が間違っている」と
実装者に言う文であり、利用者の画面には出ない — 出るとしたらホストの実装が間違っている。
`TriggerComposer` の契約検査や `HostOptions.Initialize` の呼び忘れがこれにあたる。
**日本語の直書きはどちらでもない**ので、`NoSourceLiteralCarriesJapanese` が塞ぐ。

分類 3 (英語固定) がどこを指すかも書いておく。サンプルホストが `--place-windows` や
`--culture` の解釈に失敗して残す行、`TestHost` の発火ログ、`ILogger` へ流す診断はすべて
分類 3 である。行き先は**画面ではなく診断ログ**であり、共有して grep するものなので、
実行環境の言語で語彙が変わってはいけない。`TestHost` はユーザー向け出力 (使い方・
利用ミスの理由・進捗・診断のまとめ) をリソース経由にしつつ、発火ログだけ英語のままである —
同じプロセスの中で 2 と 3 が並ぶ実例になっている。

運用規則:

- 新しいユーザー向け文字列は**必ず `.resx` / `.resw` 経由**。ハードコードは T1 の単体テスト
  (`NoSourceAssignsAUserFacingLiteral` — `XamlLocalizationTests`) が検出する
- **`src/` の `.cs` の文字列リテラルに日本語を書かない** (`NoSourceLiteralCarriesJapanese`)。
  日本語が正当に居られるのは実装内部のコメント・`*.ja.resx`・`Resources/*.ja.xml` の 3 か所
  だけで、どれも `.cs` のコードではない。この 1 本で分類 2 (リソース経由) と分類 3
  (英語固定) の**両方**を縛る — 分類ごとに別の網を張ると、`throw` 文だけを見る形のように
  例外を**返す**ファクトリを丸ごと見落とす
- `en-us` と `ja-jp` の**キー集合が一致していること**を単体テストでアサートする
  (翻訳漏れの自動検出)。2 言語の組は放っておくと必ずずれるためで、この「規律で解かず
  機械で縛る」形は本ファイルの各所で繰り返し使う
- **検査対象の表に `src/` のプロジェクトが 1 つ残らず載っていること**を別に見る
  (`EveryProjectIsEitherLocalizedOrExplicitlyExcluded`)。載せない判断は理由付きの除外表へ書く —
  「表に載っていないから緑」は実際に起き、`TestHost` のユーザー向け出力が全部日本語直書きの
  まま、どのゲートにも掛からなかった
