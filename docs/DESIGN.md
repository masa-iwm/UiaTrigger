...
- **下段は選択に従う。**複合 1 件なら埋め、それ以外を選んだら空に戻す — 下段は常に「いま押したら何が起きるか」を表す。前に選んだ複合の式が残っていると、それがこれからまとめる複合に効くように見える。**代償として順番が意味を持つ**: 選ぶより先に入力した値は、選び直した時点で消える。まとめる操作は「選んでから入力する」順なので実害は無いが、選択を足しに戻ると打ち直しになる
- **更新した後はその行を選択したままにする** (`ITriggerListEditorView.SelectRow`)。行を差し替えると選択が落ちるので、戻さないと直したばかりの複合が選ばれていない状態になり、続けて直すたびに一覧から探し直すことになる。戻した選択は View から報告し返さない — 駆動しているのは利用者ではなく presenter であり、欄は presenter が自分で書く
- **プレゼンターが実行時に引くキーはドットを含んではいけない。**ドット付きはコントロールのキーであり、WinUI の `resources.pri` では `CombineTriggersButton/Content` という経路として格納される。`x:Uid` はそれを解決するが `ResourceLoader.GetString("CombineTriggersButton.Content")` は解決できず、`MrtPickerStrings` が握り潰して**キー文字列そのもの**を返すため、ボタンに `CombineTriggersButton.Content` と出る (実際に出た)。resx 経路 (WPF / Windows Forms) はドットでも引けるので T1 は緑のまま通る — だから差し替える文言には別のドット無しキーを立てる (`CombineButtonCombine` / `CombineButtonUpdate` / `CommitButtonUpdate`)。ソースの形は `PresenterStringKeyTests` が縛る
...
- **明示 `Dispose` も借用と排他する** (B10)。借用の成立は解放要求フラグと同じワードの CAS なので、「借用中に別スレッドが Dispose した」場合の解放は最後の借用が閉じるまで遅延する。ファイナライザー経路だけを塞いでも、presenter の掃き出し (UI スレッドの Dispose) がディスパッチャの進行中呼び出しと重なる形は同じ壊れ方をする
- ファイナライザーとの競合は実行時テストにできない (再現が GC のタイミングに依存する)。だから**危険な書き方ができないこと**を源泉で固定する (`ElementBorrowTests`): 借用が必ず `using` に入っていること。「`.Borrow()` の個数を数える」形は、その場で使う `a.Borrow().Element` を素通りさせるので検査にならない。明示 Dispose との排他 (B10) だけは決定的に再現できるので、そちらは実行時テストで縛る (`ElementBorrowRuntimeTests`)
...
3 ホストが読む引数は `--pick-at` (繰り返し可 — n 枚目のピッカーに n 番目を配る) / `--culture` /
`--triggers` / `--place-windows` (物理ピクセルの `L,T,W,H`) で揃っている。
読み取りは `src/UiaTrigger.App.Shared` の `HostOptions` が 1 つ持つ。
...
- `HostOptions` は `src/UiaTrigger.App.Shared` (AnyCPU・配らない) に 1 つだけ置く。写しが同一でなかった 3 点はこう扱う: 名前空間は一本化 / ログの出口は `Initialize(Action<string>)` で注入 / WinUI だけの MRT 言語上書きは `ApplyCulture()` の戻り値を受けて WinUI 側で立てる (Windows App SDK 依存なので共有側に置けない)。**呼び忘れは静かな既定値ではなく例外**にする — 既定へ落ちると `--triggers` が効かないまま開発機の実ファイルを書き換えるところまで素通りする。**ソースのリンク共有 (`<Compile Include="..\Shared\...">`) へは戻さない** (あれは WinUI3 / x64 でプロジェクト参照ができない場合の回避としてだけ許している)
- `--place-windows` はホストが**自分の窓を自分で置く**ための口である。窓を退かす仕事を T4 のハーネスから移したのは、ハーネス側の見張りに**寿命**があったためで、止まったあとに現れた窓やフレームワークが当て直した窓は誰も退かさなかった。**幾何では直せない** — カスケードの位置はセッション中に作られた窓の数で動くので、余裕を測る相手になる定数が無い。エディタと子ピッカーの窓は**出荷するライブラリが作り**ホストが参照を持てないので、生成箇所ごとではなく自プロセス限定の窓イベント (`SetWinEventHook`) で拾う。コールバックは `[UnmanagedCallersOnly]` の関数ポインタにする — デリゲートは AOT で回収され、**落ちるのではなく黙って効かなくなる**
