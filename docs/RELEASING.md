...
| `real-uia` | T3 (実 UIA) | 観測 (`continue-on-error: true`) |
| `picker-ui` | T4 (ピッカー UI) | 観測 |
| `input` | T5 (合成入力) | 必須 (揺れ始めたら観測へ戻してよい — 1 行である) |
| `aot` | AOT 発行 + ローカライズの実行検証 + S4 | 必須 |
| `arm64-build` | ARM64 のビルドのみ (§5) | 必須 |
| `lint` | 合成入力の規約 grep 2 本 (docs/TESTING.md §3) | 必須 |

対話セッションを要するジョブは「まず観測 → 安定したら必須化」の順で扱う。
観測ジョブの赤は CI 全体を止めないので、**緑の一覧だけを見て通ったと結論しないこと**。
...
- **配布する `.nupkg` 内の `.xml` に日本語が 1 文字も無いこと**を再検査する。
  公開 API の doc は英語、非公開のコメントは日本語のままでよい、と決めてあるので、
  この 1 つの検査が「非公開メンバーの絞り込みが効いたか」をそのまま表す。ビルド出力側の
  検査 (T1) とは別に**実際に配る成果物の側**にも置く — 「絞ったファイルとは別のファイルが
  pack された」形は出力側の検査では捕まらない。`.xml` を配るはずの 4 パッケージが揃って
  いることも数える (「1 つも見つからなかったから緑」を塞ぐ。`Picker.WinUI` は
  `GenerateDocumentationFile` を立てておらず `.xml` を配らない)。
...
- **README の依存の記述と、nuspec の依存を突き合わせる。**README は
  ...
  そこを読み分けない。
- **配る `.xml` に `<b>` タグが混じっている** (`UiaTrigger.Core.xml` に 12 /
  `Picker.Core.xml` に 2 — 実測)。強調の `**…**` が HTML に化けたもので、XML doc としては
  未知のタグなので中身は表示されるが、**利用者の IntelliSense に出る**。

  範囲は追跡ファイル全体で **`<b>`/`</b>` が 1,574 組・165 ファイル**
  (`.cs` 139 / `.csproj` 10 / ja doc の `.xml` 4 / `.xaml` 4 / `.resx` 3 / `.md` 3 /
  `.yml` 2 / `.props` 2 / `.targets` 1)。同じ形で `<i>`/`</i>` が 4 組ある。**置換は機械で
  済む**ことを確かめてある:

  - 開きと閉じの数が**両方とも一致する** (`<b>` 1,574 / `</b>` 1,574、`<i>` / `</i>` は 4)
  - `.resx` の 3 件はいずれも**ファイル冒頭の XML コメントの中**で、`<value>` には無い。
    つまり**画面に出る文字列は 1 つも影響を受けない**
  - 同時に混入した他のタグは無い (`<em>` / `<strong>` / `<del>` / `<ul>` / `<a href>` は 0)。
    `<code>` の 4 件は XML doc の正規のタグなので**残す**。`&amp;` / `&lt;` / `&gt;` は
    XML の必要な実体参照であり、`.md` / `.yml` には 1 つも無いので**残す**
  - 同じ文が両方の形で在る (この文書の `**Release はドラフトで止める。**` と `release.yml` の
    `<b>` 版) ので、由来は機械変換の取りこぼしである

  `///` doc をどの形に直すかだけは判断が要る — XML doc に強調の記法は無いので、`**…**` に
  戻すか、落とすかである。**直すなら配る前に一度で直す。**版を上げるたびに配り直すものである。
